...
Last validated: 2026-07-04 (Pass 108: goal-directed remediation of the GitHub code-scanning backlog — ~204 open Trivy alerts against the built image; 3 findings, all fixed; redundant docker.io install, missing OS security updates, unactionable vendored-module scan noise)

## Summary
- Total: 84 | Open: 0 | Fixed: 83 | Invalid: 1 (machine-counted canonical `N-<n>` findings. Pass 108 added N-83–N-85 (3 findings, all fixed in-pass). Pass 107 added N-79–N-82 (4 findings, all fixed in-pass). Pass 106 fixed its own 9 findings, corrected the prior 69/1 mis-count, and canonicalized two ad-hoc Pass-53 ids — `TODO-OIDC`→`N-47`, `ENV-CLARITY`→`N-48`. One legacy invalid finding, `N-A` (Pass 1), keeps its label because its era's numeric id `N-21` was explicitly retired; it is intentionally not machine-counted.)
...
- Pass 108 (2026-07-04): goal-directed remediation of the GitHub code-scanning backlog (~204 open Trivy alerts, all against the built image). 3 new findings: 1 High (N-83: `docker.io` apt install redundant — the base image already ships the docker CLI 29.5.3 + buildx; the package dragged in dockerd/containerd/runc/ctr and ~130 alerts of Go-binary CVE surface for daemons never executed in-container), 2 Medium (N-84: no OS security-update step, so every Ubuntu package CVE fixed after the base snapshot sat open — 56 alerts; N-85: the Trivy SARIF included Go/npm modules vendored into upstream binaries, where "fixed version" means the module, not any released binary, so `ignore-unfixed` cannot suppress them and no repo action can remediate — 61 permanently-open alerts). All 3 fixed in-pass: dropped `docker.io`, deleted the base's daemon-side static binaries, added `apt-get dist-upgrade`, scoped the scan with `skip-files`/`skip-dirs` + SECURITY.md rationale. Verified: rebuilt image, binaries present/absent as intended, entrypoint PEM refusal intact, local Trivy with CI settings reports **zero** vulnerabilities (unrestricted scan: 61, all inside the two skipped vendored paths), hadolint/yamllint/pre-commit all green.
...
_None._ Pass 108 (2026-07-04) found 3 issues (1 High, 2 Medium);
all were fixed and re-verified this pass — see **Fixed → Pass 108** below.

## Fixed

### Pass 108 — 2026-07-04

#### [N-83] `docker.io` apt install is redundant and ships an unused container-runtime CVE surface
Fixed: 2026-07-04
Notes: Category: security. Area: `Dockerfile`. The base image (`ghcr.io/actions/actions-runner:2.335.1`) already installs the docker CLI (29.5.3 static bundle → `/usr/bin`) and the buildx plugin; the additional `docker.io` package existed only for the CLI and pulled in dockerd/containerd/runc/ctr/containerd-shim — never executed here because the daemon lives on the host behind socket-proxy. These binaries accounted for ~130 of the ~204 open code-scanning alerts (Go-binary CVEs in containerd/runc/docker modules). Fix: removed `docker.io` from the apt install and added a `RUN rm -f` for the daemon-side binaries the base's static bundle leaves in `/usr/bin` (dockerd, docker-proxy, containerd, containerd-shim-runc-v2, ctr, runc). Verified in the rebuilt image: all six absent, `docker --version` 29.5.3 and buildx plugin present, entrypoint PEM-refusal smoke test passes.

#### [N-84] No OS security-update step — Ubuntu package CVEs fixed after the base snapshot stay open until upstream rebases
Fixed: 2026-07-04
Notes: Category: security. Area: `Dockerfile`. The image inherited the base snapshot's Ubuntu packages verbatim; 56 alerts (curl/libcurl, ncurses, nghttp2, containerd, …) had fixed versions already published in the Ubuntu archive. Fix: `apt-get dist-upgrade -y` added to the apt layer (DL3005 added to the inline hadolint ignore with rationale comment). Verified: local Trivy scan of the rebuilt image reports zero OS-package vulnerabilities.

#### [N-85] Trivy SARIF includes vendored-module findings no action in this repo can remediate
Fixed: 2026-07-04
Notes: Category: security/reliability. Area: `.github/workflows/ci-docker.yml`, `docs/SECURITY.md`. Trivy flags Go/npm modules vendored into upstream binaries (buildx: 33 findings incl. 20 High; npm CLI's own `node_modules` under `externals/node{20,24}`: 28 findings). For these, "fixed version" refers to the module, not to any released buildx/actions-runner build, so `ignore-unfixed: true` cannot suppress them and they sit open indefinitely — alert noise that trains operators to ignore the Security tab. Fix: `skip-files` (buildx plugin) + `skip-dirs` (the two npm vendored trees) on the Trivy step, with an explanatory comment and a "What is explicitly NOT defended" paragraph in SECURITY.md; remediation arrives via Renovate-managed base-image digest bumps. OS packages, our own layers, and the docker CLI remain fully scanned. Verified: scan with CI settings reports zero vulnerabilities; the unrestricted scan's remaining 61 findings all live inside the two skipped vendored paths.
